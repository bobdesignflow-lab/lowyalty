...
$whatsappCSS = @"
        ...
        .whatsapp-float svg { width: 34px; height: 34px; fill: #ffffff; flex-shrink: 0; }
        ...
        .whatsapp-float:focus-visible { outline: 3px solid #1575CC; outline-offset: 2px; }
...
foreach ($file in $htmlFiles) {
    try {
        ...
        if ($content -match 'wp-custom-css') {
            $idx = $content.LastIndexOf('</style>')
            if ($idx -gt 0) {
                $content = $content.Substring(0, $idx) + $whatsappCSS + $content.Substring($idx)
                $cssAdded = $true
            }
        }
        ...
        if ($cssAdded -and $htmlAdded) {
            ...
            $processed++
            $rel = $file.FullName.Replace($baseDir, '').TrimStart('\')
            Write-Host "OK: $rel"
        } else {
            $skipped++
        }
...
Write-Host ""
Write-Host "DONE. Updated=$processed  Skipped=$skipped  Failed=$failed"
